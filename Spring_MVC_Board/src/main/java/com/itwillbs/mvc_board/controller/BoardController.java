...
//			System.out.println("업로드 될 파일명 : " + uuid + "_" + originalFileName);
				...
				//파일이 존재하지 않을 경우 널 스트링으로 대체(슬래시 포함)
				originalFileNames += "/";
				realFileNames += "/";
	...
	//파라미터
	/*
	 * 1) 검색타입 (기본값을 넣지 않으면 파라미터가 없어 오류가 발생하기에, 기본값을 널 스트링으로 넣어줘야 함.)
	 * 2) 검색어 (기본값을 넣지 않으면 파라미터가 없어 오류가 발생하기에, 기본값을 널 스트링으로 넣어줘야 함.)
	 * 3) 현재 페이지 번호 (INT의 경우는 기본값 1 무조건 넣어야 함.)
	 * 4) 데이터 저장 객체
	 * 
	 */
