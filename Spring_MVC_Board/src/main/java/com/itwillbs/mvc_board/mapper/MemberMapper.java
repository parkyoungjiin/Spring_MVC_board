...
public interface MemberMapper {
	...
	// 5. 회원 정보 수정 작업에 필요한 updateMemberInfo
	// => 파라미터 : 없음   리턴타입 : List<MemberVO>
//	int updateMemberInfo(MemberVO member, String newPasswd);
	// => 주의 : 복수개의 파라미터가 전달될 경우 각 파라미터를 XML에서 구별하기 위해
	// @Param 어노테이션을 사용하여 해당 파라미터의 이름을 지정해줘야 한다.
	//XML 내부에서 쿼리문 작성 시 객체는 선언된 파라미터명.변수명 형태로 접근해야 한다.
	// (ex. member.id)
	//변수는 선언된 파라미터명만으로 그대로 사용이 가능하다.
	// (ex. newPasswd)
